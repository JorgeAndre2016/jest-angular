...
describe('FakeService', () => {
    ...
    service.getDataV2().subscribe(
      {
        ...
        error: error => {
          ...
          expect(error).toEqual(new Error('getDataV2 failed: server returned code 404 with body "test 404 error"'));
          done();
        }
      }
    );
  ...
  it('should test postDataV1', () => {
    ...
    const resp = 'Techopsworld';
    const url = 'https://jsonplaceholder.typicode.com/todos/1';
    jest.spyOn(httpClientSpy, 'post').mockReturnValue(of(resp));
